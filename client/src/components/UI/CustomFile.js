...
const CustomFileUpload = styled('label')({
  ...
  boxShadow: '0px 24px 17.5px -24px #252525',  // Adjusted box shadow
});
...
const CustomFileUploadComponent = ({ onChange }) => {
  return (
    <CustomFileUpload htmlFor="file">
      <IconContainer>
        <IconSVG viewBox="0 0 24 24" fill="" xmlns="http://www.w3.org/2000/svg">
      ...
      <TextContainer>
        <Typography variant="body1" sx={{ fontWeight: 400, color: '#e8e8e8' }}>
          Click to upload File
        </Typography>
      </TextContainer>
      <input id="file" type="file" style={{ display: 'none' }} onChange={onChange}/>
    </CustomFileUpload>
  );
};
